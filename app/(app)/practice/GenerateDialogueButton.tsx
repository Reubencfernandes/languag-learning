...
export function GenerateDialogueButton() {
  ...
  function submit() {
    ...
    startTransition(async () => {
      ...
      setOpen(false);
      setScenario("");
      router.push(`/practice/dialogues/${j.id}`);
      router.refresh();
    });
  }
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <input
        ...
        placeholder="Scenario (e.g. at the airport) — optional"
        className="h-10 w-64 rounded-full border border-white/10 bg-card px-4 text-sm text-[#E1E0CC] placeholder:text-white/30 focus:outline-none focus:border-primary/50"
      />
      <button
        onClick={submit}
        disabled={isPending}
